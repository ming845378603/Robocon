...
Flag Flag_ANO_DT_Data = Not_Ready;
Flag Flag_ANO_DT_Forward_Pid = Not_Ready;
Flag Flag_ANO_DT__Pid = Not_Ready;
...
/**
  ...
  */
extern s16 targetX, targetY1, targetA;
s16 Flag_X=0, Flag_Y=0, Flag_A=0;
uint8_t state = 0;
uint8_t ANO_DT_Data_Receive_Prepare(uint8_t data)
{
    ...
    else if(state==5)
    {
		...
		else
		{
			...
			setup_Yline(targetY1);
				Get_Cycle_T(0); /*前馈控制周期10ms*/
        return Ready;
    }
    else
        state = 0;
...
/**
  ...
  * @retval None
  * @brief  发送数据
  *			移植时，用户应根据..\OBJ\USART.axf: Error: L6218E: Undefined symbol g_Speed (referred from ano_dt.o).自身应用的情况，根据使用的通信方式，实现此函数
  */
static void ANO_DT_Send_Data(uint8_t *dataToSend, uint8_t length)
{
#if (ANO_DT_UART_PORT_SELECT == 1)
